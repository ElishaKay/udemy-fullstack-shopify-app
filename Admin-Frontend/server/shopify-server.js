...
export const addShopifyUser = async (query) => {
  console.log('hit addShopifyUser function on server with query: ',query);
  ...
  if (!auth) {
    console.log('ran no auth logic')
    return null;
  }

  const user = await addUser(query, auth);
   // if (!user) {
  //   console.log('ran no user logic')
  //   return null;
  // }
  ...
  if (!token) {
    console.log('ran no token logic')
    return null;
  }

  return { token, shopUrl: query.shop };
}
